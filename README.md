...
> **个人学习项目，采用 AI 辅助开发。**
>
> - 本项目围绕作者个人的追番流程和本地环境实现，代码、测试与文档在 AI 工具（包括 OpenAI Codex）辅助下完成；需求定义、产品取舍、真实环境联调、Bad Case 分析与验收由作者负责。
> - 当前仅在 **AutoBangumi 3.2.8、Dify Cloud 远程调试和本地插件进程** 这一组合下验证，配置与实现具有明显的个人环境特征，不是开箱即用的通用方案。
> - 仓库可用于参考 Dify 工具拆分、接口白名单、预览确认、幂等校验和写后回查等设计思路。用于其他环境前，请根据实际版本、网络和业务流程自行调整并测试。
> - 这是非官方实验项目。`subscribe_ab_rss` 可能创建真实订阅并触发下载，请在可控环境中使用并妥善保管凭据。

这是一个面向个人追番流程开发的 Dify Tool Plugin，在“追番订阅 Agent”整体方案中承担确定性工具层职责；它不是 AutoBangumi 官方插件，也不是一个独立的聊天机器人或推荐 Agent。
